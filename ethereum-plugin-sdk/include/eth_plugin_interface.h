...
// Interface version. To be updated everytime we introduce breaking changes to
...
typedef struct ethPluginFinalize_t {
  ...
  const uint8_t
      *amount; // set an uint256 pointer if uiType is UI_AMOUNT_ADDRESS
  const uint8_t *
      address; // set to the destination address if uiType is UI_AMOUNT_ADDRESS.
...
// If uiType is UI_AMOUNT_ADDRESS, the amount and address provided by the plugin
// will be used If tokenLookup1 is set, the amount is provided for this token

// if uiType is UI_TYPE_GENERIC, the ETH application provides tokens if
// requested then prompts for each UI field The first field is forced by the ETH
// app to be the name + version of the plugin handling the request The last
// field is the fee amount
...
typedef struct ethPluginProvideInfo_t {
  ...
  union extraInfo_t
      *item1; // set by the ETH application, to be saved by the plugin
  union extraInfo_t *item2;

  uint8_t additionalScreens; // Used by the plugin if it needs to display
                             // additional screens based on the information
                             // received from the token definitions.
