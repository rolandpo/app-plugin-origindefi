...
static void handle_token_received(ethPluginProvideParameter_t *msg,
    ...
    // determine token addresses of curve pools based on contract address and value of i/j params
    if (context->selectorIndex == CURVE_POOL_EXCHANGE || context->selectorIndex == CURVE_POOL_EXCHANGE_UNDERLYING) {
        if (memcmp(CURVE_OETH_POOL_ADDRESS, msg->pluginSharedRO->txContent->destination, ADDRESS_LENGTH) == 0) {
            switch (msg->parameter[PARAMETER_LENGTH-1]) {
                case 0:
...
// exchange_multiple(address[9] _route,uint256[3][4] _swap_params,uint256 _amount,uint256 _expected)
...
static void handle_uniswap_exchange_single(ethPluginProvideParameter_t *msg, origin_defi_parameters_t *context) {
    switch (context->next_param) {
        case TOKEN_SENT:
            handle_token_sent(msg, context);
            context->next_param = TOKEN_RECEIVED;
